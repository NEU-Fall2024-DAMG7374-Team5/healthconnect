...
Relationships:
1. :Encounter-[:RECORDED_CONDITION]->:SNOMEDCT
...
conditionsFile_DF = (spark.read.csv(f"{SOURCE_DATASET_LOCATION}/{SOURCE_DATASET_FILENAME}", header=True, inferSchema=True))
conditionsFile_DF.printSchema()
conditionsFile_DF.show(truncate=False)
...
string_columns = [field.name for field in conditionsFile_DF.schema.fields if isinstance(field.dataType, StringType)]


# Replace nulls in numeric columns (skipping the amount columns) with -999
numeric_columns = [field.name for field in conditionsFile_DF.schema.fields if isinstance(field.dataType, NumericType)]
...
conditionsFile_DF = (
    ...
    .withColumnRenamed("START", "CONDITION_STARTED_ON")
    .withColumnRenamed("STOP", "CONDITION_STOPPED_ON")
)

# Iterate over each column, updating only those with string type
for field in conditionsFile_DF.schema.fields:
    if field.dataType.simpleString() == 'string':
        conditionsFile_DF = conditionsFile_DF.withColumn(field.name, upper(conditionsFile_DF[field.name]))

logging.info("Source Dataset Schema after data cleaning")
conditionsFile_DF.printSchema()

logging.info("Source Dataset Schema Sample Records")
conditionsFile_DF.show()
...
#Relationship = :Encounter-[:RECORDED_CONDITION]->:SNOMEDCT
logging.info("Inserting Relationship (:Encounter-[:RECORDED_CONDITION]->:SNOMEDCT)")
(
    conditionsFile_DF
    .select(
        "ENCOUNTER",
        "CODE",
        "CONDITION_STARTED_ON",
        "CONDITION_STOPPED_ON"
    )
    .write
    .format("org.neo4j.spark.DataSource")
    .mode("Overwrite")
    .option("relationship", "RECORDED_CONDITION")
    .option("relationship.save.strategy", "keys")
    .option("relationship.properties", "CONDITION_STARTED_ON, CONDITION_STOPPED_ON")
